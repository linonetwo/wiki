...
class CommandPaletteWidget extends Widget {
  ...
  refreshCommands() {
    ...
    this.actions.push({
      ...
      $tw.wiki.filterTiddlers(`[!is[system]tags[]] [is[system]tags[]] -[[${tiddler}]tags[]] +[search[${terms}]]`), true, 'tm-add-tag'),
      keepPalette: true
    });
    this.actions.push({
      name: 'Remove tag',
      ...
      (tiddler, terms) => $tw.wiki.filterTiddlers(`[[${tiddler}]tags[]] +[search[${terms}]]`), false, 'tm-remove-tag'),
      keepPalette: true
    });
    ...
    for (let tiddler of commandTiddlers) {
      ...
      let hint = (_c = (_b = tiddler.fields[this.hintField]) !== null && _b !== void 0 ? _b : tiddler.fields[this.nameField]) !== null && _c !== void 0 ? _c : '';
  ...
  render(parent, nextSibling) {
    ...
    this.execute(); // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name '$tw'.
    ...
    $tw.rootWidget.addEventListener('open-command-palette', e => this.openPalette(e)); // @ts-expect-error ts-migrate(2304) FIXME: Cannot find name '$tw'.
  ...
  addResult(result, id) {
    let resultDiv = this.createElement('div', {
      ...
      innerText: this.translateCaption(result.caption || result.name)
    });

    if (result.hint !== undefined) {
      let hint = this.createElement('div', {
        className: 'commandpalettehint',
        innerText: this.translateCaption(result.hint)
      });
  ...
  defaultProvider(terms) {
    this.hint.innerText = 'Search tiddlers (⇧⏎ to create)'; // @ts-expect-error ts-migrate(7034) FIXME: Variable 'searches' implicitly has type 'any[]' in... Remove this comment to see the full error message
