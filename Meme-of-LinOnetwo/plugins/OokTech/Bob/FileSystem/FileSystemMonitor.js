  ...
  if ($tw.node && $tw.settings.disableFileWatchers !== 'yes') {
    ...
    */
    $tw.Bob.WatchFolder = function (folder, prefix) {
      ...
      try {
        $tw.Bob.Wikis[prefix].watchers[folder] = fs
          .watch(folder, function (eventType, filename) {
            filename = filename || '';
                ...
                if (fileStats.isDirectory()) {
                  ...
                  if (['.tid', '.meta'].indexOf(fileExtension) !== -1) {
                    ...
                    if (tiddlerObject.tiddlers[0]['title']) {
                      ...
                      let newTitle = $tw.syncadaptor.generateTiddlerBaseFilepath(
                        tiddlerObject.tiddlers[0].title,
                        prefix
                      );
