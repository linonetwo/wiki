...
1. filter ，可以和 wikitext 结合起来写渲染模板，也可以用于「[把加了 私有 标签的笔记保存到私有仓库里面](https://onetwo.ren/wiki/#%24%3A%2Fconfig%2FFileSystemPaths)」等功能
...
```lisp
[tag[私有]addprefix[private-MyTiddlyWiki/tiddlers/]] [tag[$:/tags/trashbin]addprefix[private-MyTiddlyWiki/tiddlers/]]
```

意思就是把加了 `私有` 这个标签的文件保存到 `/PublicWiki/tiddlers/private-MyTiddlyWiki/tiddlers` 里面。还有放进垃圾桶的文件也如是。
...
如果你给一个 Tiddler 加上 私有 这个 tag （你也可以通过修改 `$:/config/FileSystemPaths` 来改它），TiddlyWiki 就会把你加到 Wiki 里的内容保存到刚刚创建的私有仓库里的 tiddlers 文件夹里了。
...
如果你给一个 Tiddler 加上 私有 这个 tag，TiddlyWiki 就会把你加到 Wiki 里的内容保存到刚刚创建的私有仓库里的 tiddlers 文件夹里了，而且倒计时三十分钟后会自动备份到 Github 上你的线上私有仓库里。
